...
import { q, withTx, ensureMiscForWorkspace, type WorkspaceRow } from '../db.js';
...
workspacesRouter.post('/', async (req, res, next) => {
  try {
    ...
    const maxRow = (
      await q<{ m: number }>('SELECT COALESCE(MAX(position), -1) as m FROM workspaces')
    ).rows[0];
    const id = nanoid();
    await q('INSERT INTO workspaces (id, name, position) VALUES ($1, $2, $3)', [
      id,
      parsed.data.name,
      maxRow.m + 1,
    ]);
...
workspacesRouter.patch('/:id', async (req, res, next) => {
  try {
    ...
    const row = (await q<WorkspaceRow>('SELECT * FROM workspaces WHERE id = $1', [req.params.id]))
      .rows[0];
    res.json(row);
  } catch (err) {
    next(err);
...
workspacesRouter.delete('/:id', async (req, res, next) => {
  try {
    ...
    const count = (
      await q<{ c: string }>('SELECT COUNT(*)::text as c FROM workspaces')
    ).rows[0];
  ...
  try {
    ...
    await withTx(async (client) => {
      for (let i = 0; i < parsed.data.orderedIds.length; i++) {
        await client.query('UPDATE workspaces SET position = $1 WHERE id = $2', [
          i,
